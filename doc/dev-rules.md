...
### 1.2 GitHub 发布默认同步阿里云

项目中说“发布到 GitHub”或“发版”时，默认含义是：本机发布前测试通过后，提交并 push 到 GitHub，
随后使用唯一发布脚本同步发布到阿里云，并完成线上健康检查。无需每次额外说明“发布到阿里云”。
只有明确说“只推 GitHub”时，才跳过云端发布；未通过本机测试时不得发布。
...
已有功能和稳定行为默认视为受保护契约。开发新功能时，不得以“顺便优化”或“顺手整理”为理由改变未纳入本次需求的模块、流程或对外行为。

1. 开始修改前，先明确本次任务的目标模块、允许修改的文件范围，以及明确不应受到影响的模块和行为。复杂任务写入 `next.md`、开发计划或设计文档；小范围任务至少在任务说明中保留这份边界。
2. 只修改实现当前需求所必需的代码。无关的重构、格式化、命名调整、依赖升级、接口变更和目录迁移不得混入功能修改；确有必要时，必须说明与当前需求的直接关系、影响范围和验收方式。
3. 跨模块修改必须先确认模块边界、依赖方向和契约影响。需要改变其它模块行为时，必须同步更新相应需求/设计、消费端和测试；不能通过扩大共享状态、旁路调用或复制逻辑绕过边界。
4. 修改前先读取相关实现、历史决策和现有测试，并建立可重复的回归基线。仅凭“代码看起来可以更好”不足以证明稳定行为需要改变。
5. 完成后检查目标模块、直接消费者和明确的非目标模块；至少覆盖正常路径、关键边界和原有回归测试，再按项目总门禁执行验证。测试只覆盖实际运行过的范围，不得把局部通过表述为全仓无回归。
6. 提交前检查 diff 是否超出已确认范围。发现非目标改动没有需求、证据或测试支撑时，应删除或拆出；如果无法安全隔离，先暂停并重新确认范围，不把稳定功能的破坏当作优化结果。
