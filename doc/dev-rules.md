...
本地教师人工验收账号固定为：用户名 `teacher`，密码 `teacher`。这组凭据只适用于本地
开发/人工验收数据库，严禁用于生产环境；启动完成后必须实际验证该账号能够登录。
...
3. 固定的本地专用账号 `teacher` 是唯一例外：账号不存在时创建，已存在但密码不确定时，
   允许只将它修复为本地密码 `teacher`，并验证登录成功。不得对其它已有教师账号执行会
   覆盖密码的 seed 或重置；生产数据库严禁使用这组固定凭据。
...
- 本地默认数据库是 `v1/backend/knownmap-v1.db`，由 `v1/backend/.env` 的
  `DATABASE_URL` 决定；启动服务不得默默切换到临时数据库。临时数据库只允许用于明确
  隔离的测试，并必须在验收说明中标明。
- 如果本地课程信息缺失，先确认本地服务已停止，再备份当前本地数据库，之后从阿里云
  `/var/lib/knownmap/knownmap.db` 创建一致性副本并下载到本地；必须通过 SQLite 完整性
  检查、哈希或等价校验后才能替换本地库。
- 云端数据库只读，不得为了本地登录或测试修改云端账号、密码、课程和会话数据。导入后
  只允许在本地副本上将专用验收账号修复为 `teacher` / `teacher`；如需清除本地会话，
  也只能操作本地副本。
- 导入后执行本地迁移并验证课程、课节和 `teacher` 登录；数据库导入不等于文件资产同步，
  若课程引用了云端文件资产，必须另行确认资产存储是否需要同步。
