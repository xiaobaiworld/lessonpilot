...
3. 教师草稿、课程发布快照、授权资格、学生已安装课程和本机学习状态各有唯一责任方；独立课程版本属于后续演进；
...
| 视频 | B 站视频只作为课节播放宿主和时间触发来源；互动节点图片、音频、视频是独立课程资源，按 `assetId` 管理 | `SCOPE-05`、`D-V1-016`、产品长期边界 |
| 学生数据 | 课程、回答和学习状态默认只在学生当前机器 | `SCOPE-06`、`SEC-PRIV-001` |
| 字幕 | 原始字幕在教师本机解析；教师明确保存草稿后进入服务端草稿和发布快照，并可进入教师 `.kmcourse`；不进入学生课程包或插件 | `DATA-CONTENT-005`、`INT-FILE-003`、`D-V1-014` |
...
- 教师字幕文件先在浏览器会话内解析；教师明确保存草稿后，原文进入服务端草稿和发布快照，关闭页面不影响已保存副本。
...
| 制作与发布 | 草稿、四类节点校验、节点资源引用、预览版本、课程发布快照和历史；独立课程版本操作后置 | 学生本机进度、B 站播放视频和节点媒体文件本体 |
...
`node tools/v1-module-check.mjs` 据此检查后端代码。
...
教师授权码管理沿用教师工作台已发布课程卡片进入的独立 `AccessCodesPage`，不新增全局授权中心。
页面内同时承担创建、列表、详情和批量状态管理：

1. 创建栏以当前课程为默认范围，可以继续选择当前教师自己的其它已发布课程及指定课节；
2. 创建栏提交数量、领取窗口、授权项有效时间、接收人和教师备注；
3. 列表逐条展示完整码、范围、接收人、状态和领取摘要，复选框选择后由服务端执行批量状态动作；
4. 详情只返回当前教师可管理的记录和脱敏状态历史；接收人记录不进入学生端数据流。

授权与交付模块拥有授权码、授权项和领取记录，负责范围归属、状态转移、幂等、批量事务和学生未来在线资格。
运行与审计模块拥有 `OperationAudit`，授权模块通过其应用服务记录和读取授权操作摘要，不直接查询审计表。
工作空间与课程模块只通过现有课程/课节应用服务提供教师可见摘要和归属校验。
...
领取窗口或授权项时间到期属于服务端计算的时间状态，不替代教师手工状态。冻结和作废都只阻止未来在线
领取、重新下载和更新，不删除学生已安装课程。
...
- 不持久化密码或授权码原文；字幕原文只在教师明确保存后进入草稿、发布快照和教师 `.kmcourse`，不进入学生课程包或插件；
- 不复用学生本机学习状态生成教师报表。

节点弹窗中的窗口大小、位置和样式是节点级 `presentationHints`，不是课程级或播放器级设置。教师端预览
从当前 `dialogNode` 读取这些参数和结构化 `content`，使用共享解析规则映射到预览 class；它只通过教师资源
接口回显节点媒体，不访问 B 站播放器，也不把教师预览地址写入课程包。

### 8.3 管理员应用

- 使用独立管理员会话和入口，不接受教师 Cookie 作为管理员身份；
- 只展示接入、账号状态、必要业务摘要、操作记录和受限支持信息；
- 需要课程正文的异常处置必须有独立授权和审计；当前只通过显式的课程包管理区，在指定目标教师后开放导入/导出；
...
| 部分 | 职责 |
| --- | --- |
| 课程库界面 | 领取、自动安装/更新、课程与课节浏览、删除、重置及错误恢复 |
