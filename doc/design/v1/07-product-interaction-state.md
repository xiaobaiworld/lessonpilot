...
发布成功后显示课程的 `courseId`/`releaseId` 和下一步“生成授权码”入口，但不自动创建授权码；独立课程身份和草稿消费后置。

已发布课程的“修改本版本”和“增加版本”属于后续版本操作。本阶段保留已有课程管理入口；“生成授权码”按课程创建课程级授权，
服务端绑定 `course_id`，学生兑换或更新时自动取得该课程最新可交付版本。

### 5.5 创建与交付授权码

1. 教师从已发布课程卡片点击“生成授权码”；页面展示当前课程、课节/节点范围、领取窗口和授权项有效期。
...
5. 生成成功后，授权记录明确显示该授权码对应的课程；兑换/更新结果显示服务端实际解析的 `releaseId`；终止、归档或停止继续分发前显示将影响首次领取、重新下载和更新的范围。
