...
状态：已于 2026-08-22 通过人工审核；2026-08-24 按 `FR-LIB-014` 增补公开学生插件使用说明页及链接导航入口；2026-08-27 增补教师登录后课程总览及学生作答统计接口预留，不改变插件分发方式
...
2. 首页显示按“草稿 / 已发布 / 已归档”分组的课程列表、每门课程当前状态和一个主要动作：继续编辑、管理课程或创建授权。
   已发布课程同时展示已发布互动节点数、已发授权码数和已领取浏览器设备数；学生作答位置读取课程列表接口的
...
5. 首页总览只展示服务端已确认的课程、发布和领取聚合；学生作答统计字段已在接口中预留但当前返回 `null`，学生回答与学习状态当前只保存在学生本机，不进入教师首页统计。
