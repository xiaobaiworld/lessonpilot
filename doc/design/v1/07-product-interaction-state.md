...
状态：已于 2026-08-22 通过人工审核；2026-08-24 按 `FR-LIB-014` 增补公开学生插件使用说明页及链接导航入口；2026-08-27 增补教师登录后课程总览及学生作答统计接口预留；按 `D-V1-018` 增补教师工作台课程版本操作和版本级授权码，不改变插件分发方式；2026-08-29 按 `D-V1-023` 记录学生陪伴形象后续候选，不改变当前 v1 交付范围；2026-09-01 按 `D-V1-027` 增补本地试用申请交互；2026-09-02 按 `D-V1-033` 将学生说明页真源收进 `/student/` 命名空间并保留旧入口；2026-09-03 按 `D-V1-020` 同步授权码管理页面实现和生产验证
...
“按钮点击过”不等于“保存完成”，“请求发出”不等于“课程已安装”。首次兑换在课程包校验通过后自动安装，
不设置额外的安装确认步骤。

### 2.3 高影响操作先说明再确认

发布、创建授权码、终止继续分发、归档课程、导入课程、删除本机课程和更新，都必须在执行前
...
| 管理员应用 | 平台管理员 | 创建/停用/恢复教师、受控支持、状态核对，以及指定目标教师后的课程包管理 | 代替教师编辑课程、查看教师密码、未指定目标教师时读取课程正文 |
| 教师应用 | 教师 | 管理自己的课程、课节、脚本、预览、发布和授权 | 学生本机学习、跨工作空间对象、远程删除学生课程 |
| 学生插件入口 | 学生 | 输入授权码、查看课程摘要、自动安装、查看本机课程 | 注册账号、进入教师工作台、选择服务端课程绕过授权 |
...
2. 创建栏默认选中当前课程，使用“添加授权课程”添加当前教师自己的其它已发布课程，并选择整门课程或指定课节；页面不预先展开所有课程的课节。
...
6. 每条已生成记录提供与“复制”一致的轻量描边“授权码 + 指南”按钮，按钮下说明复制内容。它不重新生成授权码，只把该记录已有的课程名称、完整授权码和 `https://knownmap.com/student/guide.html` 整理成一段文字并复制到剪贴板；复制失败时保留可手动选择的文案预览。
7. 每条记录展示领取设备数、首次领取和最近领取时间，不展示本机标识摘要、学生本机学习状态或学生回答。
8. 列表支持复选框、全选和批量冻结/恢复/作废；批量作废前显示不可恢复和不删除记录的影响。
9. 授权记录明确显示对应 `course_id`；兑换/更新仍由服务端解析实际 `releaseId`。冻结或作废前显示对首次领取、
   重新下载和更新的影响。
10. 冻结可恢复，作废不可恢复；作废只改变服务端状态，不删除授权码、接收人、范围或领取历史。
...
3. 解压、Chrome 安装和选择解压文件夹的关键动作使用带说明文字的截图或示意图，截图有替代文本，图片无法加载时不影响文字流程；
4. 下载地址继续使用 `/downloads/student-plugin/knownmapplugin.zip`，不建立第二个插件分发入口；
5. 安装完成后说明把 KnownMap 固定到 Chrome 工具栏、打开插件首页、领取授权码、打开 B 站课节和开始互动的下一步，但页面不代替插件执行课程兑换；
6. 提供安装成功判断、更新说明和常见问题，失败时保留文字恢复路径；
7. 页面不要求登录、不读取插件或学生本机状态；
8. 页面真源使用 `/student/guide.html`，`/student-guide.html` 仅作为兼容入口；页面底部再次提供下载入口和返回销售页入口，窄屏下按单列顺序展示。
...
stateDiagram-v2
  ...
  Redeeming --> Installing: 服务端返回授权摘要/课程包
  ...
  InstallError --> CodeEntry: 保留原课程库并重试
...
- 整门课程或指定课节/节点范围；
- 首次安装还是更新；
- 将保存到当前浏览器本机及可能替换的同课程版本；
- 当前授权不能得到的内容不列入“已取得”。

首次兑换不设置安装前确认；课程包完成校验后自动写入当前课程库。安装失败保留旧版本和其它课程。
...
教师课程配置导入/导出属于已接受但低于核心运行闭环的能力：课程包导出为 `.kmcourse`，包含节点辅助媒体，不包含 B 站主视频本体；导入一定先预览和确认，创建新草稿，不覆盖原课程、不自动发布、不创建授权。当前管理员可以在显式课程包管理区代指定教师执行操作；教师类型能力由后续服务端策略决定。学生课程本地文件则继续按后续候选处理，不在当前核心交互中伪装成跨设备恢复。
...
| `complete` | 节点真正完成 | 庆祝姿态，叠加一条小鱼干奖励和短暂庆祝装饰 | 播放一次独立的老虎庆祝声 |
...
公开试用申请改为销售页链接到的独立本地表单页面。销售页只负责说明当前能力和提供入口；申请人无需登录，独立表单页负责展示收集用途、必填/选填字段、留言提交状态，服务端负责保存申请正文。提交成功只显示明确成功结果，不发送邮件或其它通知；申请不会自动创建教师账号。

当前本地表单使用销售页统一视觉，并采用更接近日常留言板的表达，标题为“给 KnownMap 留个言”。表单不额外堆叠填写说明，提交成功后用感谢图标和温暖文案回应，不在公开页面展示账号创建边界。题目按称呼、联系方式、课程类别、视频状态、
B 站课程链接、字幕情况、教学问题、产品验证问题的顺序展开；B 站课程链接和产品验证问题为选填，课程类别为必填文本。
B 站课程链接保持空白也可以提交；视频状态和字幕情况默认显示中性的“还没确定”，用户可以按实际情况修改。
B 站链接填写时只做简单格式校验；限流或轻量防刷拦截时保留可恢复提示。

品牌背景源文件保存为 `v1/site/assets/trial-intake-background-v1.png`。背景只承担品牌氛围，不承载文案，
中央区域必须保持低对比度，确保本地表单卡片和正文在桌面端、窄屏裁切后仍可读。

销售页入口固定为“在线填写试用申请”，点击后进入独立的本地试用申请页面；入口辅助说明为“写下课程情况和想解决的问题。”。表单页面打开或点击提交
不等于申请成功，只有 KnownMap 服务端确认保存后才显示成功，并进入管理员的“待联系”列表。
