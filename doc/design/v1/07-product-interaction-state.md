...
状态：已于 2026-08-22 通过人工审核；2026-08-24 按 `FR-LIB-014` 增补公开学生插件使用说明页及链接导航入口；2026-08-27 增补教师登录后课程总览，不改变插件分发方式
...
2. 首页显示按“草稿 / 已发布 / 已归档”分组的课程列表、每门课程当前状态和一个主要动作：继续编辑、管理课程或创建授权。
   已发布课程同时展示已发布互动节点数、已发授权码数和已领取浏览器设备数；学生作答位置显示为“当前未接入”，不得用
   领取数冒充学生作答数。
3. 会话失效时保留已经确认保存的课程数据，要求重新登录；未保存输入明确标记为临时，不能伪装成已保存。
4. 账号停用、网络不可用或服务故障时显示原因和恢复动作，不把课程列表空白显示成课程被删除。
5. 首页总览只展示服务端已确认的课程、发布和领取聚合；学生回答与学习状态当前只保存在学生本机，不进入教师首页统计。
