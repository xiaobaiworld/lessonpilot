...
状态：已于 2026-08-22 通过人工审核；2026-08-24 按 `FR-LIB-014` 增补公开学生插件使用说明页及链接导航入口；2026-08-27 增补教师登录后课程总览及学生作答统计接口预留；按 `D-V1-018` 增补教师工作台课程版本操作和版本级授权码，不改变插件分发方式
...
发布成功后显示新独立课程的 `courseId`/`releaseId` 和下一步“生成授权码”入口，但不自动创建授权码；被发布的草稿从草稿区移除。

已发布课程版本卡片提供三个动作：

1. “修改本版本”：确认后移除该已发布课程，将内容退回草稿区；原已发布课程不再保留。
2. “增加版本”：确认后保留原已发布课程，同时以当前内容建立一份新的草稿；之后发布时生成新的独立课程。
3. “生成授权码”：针对当前已发布课程版本生成版本级授权码；页面必须展示所绑定的课程版本，不得默认绑定同名课程或其它版本。

“修改本版本”和“增加版本”必须在确认页明确说明原已发布课程是否保留。失败、取消或结果待确认时，不能误删原已发布课程或伪造草稿。

### 5.5 创建与交付授权码

1. 教师从已发布课程版本卡片点击“生成授权码”；页面展示当前课程版本、课节/节点范围、领取窗口和授权项有效期。
...
5. 生成成功后，授权记录明确显示该授权码对应的课程版本；终止、归档或停止继续分发前显示将影响首次领取、重新下载和更新的范围；已安装到学生本机的课程不承诺远程删除。
...
课程库按课程名称、课节数量、已取得的课程版本、授权范围和本机学习状态摘要展示。不同发布版本按独立课程身份处理；选择一门课程后，学生可以：

- 打开匹配视频并进入学习；
- 查看当前版本和本机状态；
- 在后续升级关系能力提供后检查版本升级；当前版本级授权不自动跨到其它课程；
- 明确删除本机课程及其学习状态。
