...
范围和操作审计；当前课程包管理区是这一受控路径，必须先指定目标教师和课程，管理员身份仍不自动获得教师编辑权限。
...
- 授权码使用高熵随机值，原文只在所属教师认证后的管理页面和学生输入请求的短暂生命周期内存在；创建后允许所属教师再次查看和复制，复制学生文案不创建新授权码。
...
- 课程包和教师课程文件只接受声明版本、封闭字段、允许节点类型、稳定身份、显式顺序和受支持视频引用；`.kmcourse` 还必须校验每个节点辅助媒体的路径、字节数和 SHA-256。
- 字段中的文本按数据展示；不作为 HTML、JavaScript、模板、Shell 命令、SQL、文件路径或动态 import 执行。
- 字幕解析限制文件大小、编码、句数和时间戳，真实错误返回具体问题；解析失败不覆盖已有草稿。
- 课程包安装、更新、课程导入均使用 staging → 完整校验 → 自动提交 → 原子写入；失败清理未提交候选，保留
  最近有效版本和其它课程。
