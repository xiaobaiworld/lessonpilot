...
状态：已于 2026-08-22 通过人工审核；本文件把四类安全底线和第一阶段运行门禁落为最小可执行设计，不建设企业级安全平台
...
flowchart LR
  ...
  Feishu[飞书公开表单] -->|人工读取| Ops[获准运营/管理员]
```

默认不信任：浏览器表单、URL 参数、页面消息、content script、网络响应、课程包、字幕文本、导入文件、旧本机
存储和外部表单地址。可信来源只包括服务端已验证会话、服务端数据库事务、已签名/摘要校验的发布构建和插件
...
- 课程包安装、更新、课程导入均使用 staging → 完整校验 → 学生/教师确认 → 原子提交；失败清理未提交候选，保留
  最近有效版本和其它课程。
- 课程包中的 `courseId`、`releaseId`、课节顺序、节点身份和视频引用必须互相一致；不能由客户端自行指定课程 ID
  扩大授权范围。
...
- 领取/更新只发送完成授权所需的本机标识、证明和课程版本意图；不发送完整字幕、课程运行状态、学生回答或学习
  进度。
...
销售页说明收集用途、第三方表单处理、必填/可选字段和人工联系入口。飞书是申请正文真源；KnownMap 不复制正文
到普通日志、课程正文或学生数据，不因提交申请自动创建教师账号。只有获准运营人员读取最小申请内容并进行必要跟进。
...
| 故障处置 | 发布中断、数据库不可用、外部表单失败、B 站失配和插件更新失败的可见状态与恢复记录 |
