...
6. **默认最少持久化**：服务端不保存完整字幕、学生回答或学习状态；客户端不把密码、会话令牌或授权码原文
   写入 Web Storage、课程包或业务记录，认证 Cookie 只按 08 的会话边界存在。
...
| --- | --- | --- | --- |
| FastAPI + SQLite | 账号、会话、工作空间、课程、草稿、课程发布、授权码、授权项、领取记录、操作审计 | 有效资格、状态摘要 | 完整字幕、学生回答、学习进度、密码或授权码原文 |
| 教师浏览器会话 | 当前字幕解析结果、未保存表单状态 | 时间线索引、预览投影 | 自动上传完整字幕、长期保存会话凭证 |
...
完整字幕、课程/课节标题、BVID、页面选中项、缩放、弹窗位置和授权信息不进入 `content`。草稿保存是整份聚合
替换：修订不匹配或任一节点无效时不写入；v1 不为每次键盘输入建立服务器历史版本。
...
这些对象关闭页面后可以消失。自动恢复若使用浏览器临时存储，也必须有明确过期、清理和敏感字段白名单；完整
字幕和未保存输入不得因“恢复方便”成为未声明的长期数据真源。保存成功只以服务端返回的新 `revision` 为准。

### 11.2 教师课程导入导出文件

`TeacherCourseFile` 是自描述的 input/output 契约，不是数据库实体。它包含文件格式版本、来源类型（已保存草稿或
发布版本）、来源课程/发布的参考信息、课程结构、课节、视频引用和节点教学语义；不包含视频文件、完整字幕、
密码、会话、授权、学生数据或页面未保存内容。
...
4. 服务端模型不包含完整字幕或学生学习数据，本机模型不形成跨设备同步承诺；
