...
状态：已于 2026-08-22 通过人工审核；第 14 节两项工程取舍均已采用；2026-08-27 同步节点预览实现；按 `D-V1-018` 增补独立课程版本、版本级授权和后续课程升级关系；2026-09-01 按 `D-V1-027` 增补本地试用申请对象
...
3. 课程制作、独立课程版本发布、版本级授权领取和本机学习使用什么身份连接；
...
6. **默认最少持久化**：服务端不保存学生回答或学习状态；字幕原文经教师明确保存草稿动作持久化到
   `ScriptDraft`/`ReleaseLessonSnapshot`（`DATA-CONTENT-005`），但不进入学生交付课程包或插件；教师 `.kmcourse` 是独立迁移格式，可携带已保存字幕；客户端不把
...
| --- | --- | --- | --- |
| FastAPI + SQLite | 账号、会话、工作空间、课程、草稿（含教师保存的字幕原文）、课程发布快照（含字幕）、授权码、授权项、领取记录、操作审计、试用申请正文和跟进状态 | 有效资格、状态摘要 | 字幕进入学生交付课程包（教师已保存字幕可进入独立 `.kmcourse`）、申请正文进入普通日志/课程正文/学生数据、学生回答、学习进度、密码或授权码原文 |
...
erDiagram
  ...
  TrialApplication ||--|| TrialFollowup : has_followup
  TrialFollowup }o--o| TeacherAccount : may_link
```

`OperationAudit` 使用受控弱关联，避免在关系图中伪装成强外键；`TrialApplication` 是本地申请正文真源，`TrialFollowup` 只保存其运营状态、更新者、时间和可选教师弱关联。恢复执行记录的位置见第 10 节。
...
| 对象 | 稳定身份与关键字段 | 基数与约束 | 生命周期/分类 |
| --- | --- | --- | --- |
| `Course` | `course_id`、`workspace_id`、`version_family_id`、`source_course_id`、`source_release_id`、`version_number`、标题、说明/目标、修订、生命周期状态、时间戳 | 一个工作空间多门独立课程；“增加版本”创建新的课程根对象并记录来源 | 身份不随标题或课节编辑变化；并存版本不得复用 `course_id`；归档与内容争议暂停分开 |
...
- 发布成功会为新课程版本建立新的 `course_id`；课程标题、视频、节点内容或版本号不能单独用来复用旧课程身份；
...
（`DATA-CONTENT-005`、`D-V1-014`）；课程/课节标题、BVID、页面选中项、课程级缩放和授权信息不进入
`content`。节点级窗口大小、样式和位置属于 `nodes[].presentationHints`，随节点内容一起保存，不能误认为
教师页面的全局 UI 状态。草稿保存是整份聚合替换：修订不匹配或任一节点无效时不写入；v1 不为每次键盘输入建立
服务器历史版本。

### 7.2 `InteractionNode`

互动节点是草稿或发布快照内部的领域对象，不因实现方便被等同为数据库行。当前课程包中的公共字段包括：

- 稳定 `id`；同一课节草稿内唯一，编辑非身份字段时不改变，删除后不复用；
- `enabled`、`family`、`interaction`、`anchor` 和 `effects`；当前四类节点均要求 `time_cross` 与暂停效果；
- `title` 和版本化的 `content: RichPageDocument`；正文不再使用 `display.body`、`display.richBody` 或 HTML 回退；
- 类型专属的 `interactionData`，重点标注为 `null`，题型保存选项、答案或反馈等必要数据；
- 可选 `presentationHints`；`anchor.captionId` 只用于教师编辑上下文，不携带完整字幕正文；
- 未知类型、未知文档版本、未知内容块或资源引用错误必须安全拒绝，不能静默降级。

v1 只允许重点标注、选择题、填空题和问答题四类。节点内容中的图片、音频和视频只保存不可变 `assetId`，资源元数据位于同一草稿/发布快照及课程包的 `assets` 清单；教师端的上传或 URL 导入由服务器生成资源身份并保存二进制，URL 不成为学生运行时依赖。B 站 `videoRef` 仍是课节播放引用，不是节点媒体。数据库层只保存已经过共同领域校验的结构，不允许任意 JSON 绕过类型规则；资源二进制不进入 SQLite，而进入服务器资源存储。

`presentationHints` 当前契约为：
...
它是节点的可选展示提示，不改变 `node.id`，也不创建独立节点表。教师编辑时先进入 `dialogNode` 的临时状态，
保存节点后进入页面内的 `nodes` 聚合，课节保存草稿后才写入 `ScriptDraft.content`。切换节点类型时保留已有
`presentationHints`，因为窗口展示与题型无关。

### 7.2.1 节点媒体资源

`AssetRecord` 是节点内容引用的资源元数据值对象，不单独拆成 v1 业务表。它至少包含 `assetId`、`kind`、`mimeType`、`byteSize`、`sha256` 和 `sourceType`，可选尺寸、时长和替代文本。教师上传文件或提交 URL 后，服务器完成安全校验、生成 `assetId`、保存二进制，并返回该记录；编辑器随后把 `asset://<assetId>` 写入富文本。

服务器文件存储的元数据与资源归属由资源存储层维护，草稿只保存 `assets` 清单和节点引用。发布时清单随节点复制到不可变的 `ReleaseLessonSnapshot`；它与 `CourseRelease` 一起构成正式发布版本，后续草稿修改不影响该快照。课程包只投影元数据和稳定引用，具体字节由交付端按 `assetId` 获取。
...
```mermaid
flowchart LR
  Draft[当前课程草稿] --> Validate[整门课程校验]
  Validate --> PublishedCourse[已发布课程]
  PublishedCourse --> Release[CourseRelease 最新快照]
  Release --> Snapshots[ReleaseLessonSnapshot × N]
  Snapshots --> Package[按课程级授权范围投影最新课程包]
```

| 对象 | 稳定身份与关键字段 | 关键规则 |
| --- | --- | --- |
| `CourseRelease` | `release_id`、`course_id`、发布意图 ID、课程级元数据快照、发布时间、发布教师、源修订摘要、权利确认、Schema 版本 | 当前课程的一次原子发布父事实；创建成功后不可变；授权码按 `course_id` 自动解析最新可交付快照 |
| `CourseVersionOperation` | `operation_id`、教师、源 `course_id`/`release_id`、`mode`、幂等键、结果草稿 `course_id`、源版本是否保留、时间 | 服务端版本事务的幂等事实；同一教师和幂等键只能产生一个结果 |
| `ReleaseLessonSnapshot` | `release_lesson_id`、`release_id`、原 `lesson_id`、排序、课节元数据、视频引用、节点内容、摘要 | 同一发布下每个课节实例恰有一份；内容相同也不合并；不可单独成为课程身份 |
| `CourseFamily`（后续） | `course_family_id`、工作空间、名称/说明、状态 | 只用于组织存在升级关系的独立课程，不是授权或交付对象 |
| `CourseUpgradeRelation`（后续） | `relation_id`、旧 `course_id`、新 `course_id`、确认教师、状态、时间 | 明确表达可升级方向；不能以标题或版本号推导，不能改变任一课程身份 |
| `ReleaseAvailability` | `release_id`、可交付状态、原因码、状态修订、操作者与时间 | 与不可变内容分离；损坏、争议或受控暂停时阻止新交付，不改写快照 |

数据库必须保证 `course_id + release_number` 唯一，并在一个事务内写入父发布及全部课节快照。
发布意图仍需幂等，不能因重复请求产生两个相同版本。当前发布只要求课程至少有一节课节；写入失败时不产生发布。
“修改本版本”复制快照后移除该课程发布快照并在原 `course_id` 重建草稿；“增加版本”保留源发布、创建新 `course_id` 草稿并递增课程族业务版本号。两个动作都由一个应用服务事务完成并写入 `CourseVersionOperation`，不得由前端组合导出、导入或归档端点。
...
| 对象 | 稳定身份与关键字段 | 关键规则 |
| --- | --- | --- |
| `AccessCode` | `access_code_id`、工作空间/管理锚点、凭证摘要、生成意图、显示尾号、领取起止时间、使用上限、状态、接收人记录、创建者与时间 | 数据库不保存原文；所属教师查询时由服务端密钥和生成意图重建并复核摘要；冻结、作废、到期和使用上限分开；接收人记录只属于教师管理侧 |
| `GrantItem` | `grant_item_id`、`access_code_id`、`course_id`/课节/节点范围、现实有效起止时间 | 当前范围根对象是 `course_id`；兑换或更新时自动解析该课程最新可交付发布版本；版本级授权根对象后续再增加；随授权码创建后不可原地改范围 |
| `Redemption` | `redemption_id`、`access_code_id`、本机学习标识摘要、首次/最近领取时间、领取时范围摘要 | `(本机学习标识摘要, access_code_id)` 唯一；不同标识数量由服务端从成功关系计算；重复请求返回同一业务结果 |
...
授权码和授权项在创建事务完成后不允许普通编辑；范围或时间设错时作废原来源并新建授权，避免已领取设备的资格被静默扩大或缩小。
`AccessCode.status` 的 `active`、`frozen`、`terminated` 分别表示有效、可恢复冻结和不可恢复作废；冻结/作废都保留授权码、
授权项、接收人、领取关系和审计历史。接收人显示信息与教师备注可以独立修改，但不改变授权范围、时间和学生资格。
当前 `LocalIdentity` 是插件本地生成的随机浏览器安装标识，不是硬件指纹；授权码的使用数量按不同标识的成功
`Redemption` 关系计算。同一标识重复领取不增加数量，新的第 21 个标识必须在创建关系和返回课程包前被拒绝。

`Redemption` 只代表已经成功建立的领取关系。无效码、超时或拒绝请求不创建领取关系；为限速、安全排障和识别
响应丢失保留的失败摘要属于受限诊断/审计记录，并使用请求幂等标识，不能混入有效资格计算。

资格计算必须分别回答：是否可以首次领取、重新下载和更新。当前课程级授权码回答 `course_id` 的交付资格，服务端自动解析其最新可交付发布版本；
终止或到期只使对应来源不再贡献未来在线资格，其它来源继续有效，已经安装在学生设备的课程也不被远程改写。
独立课程版本的授权、课程族升级关系和跨独立课程更新，待 `CourseFamily`/`CourseUpgradeRelation` 完成后再实现。
...
授权码创建、接收人记录修改、冻结、恢复、作废及批量状态动作均进入该审计事实；审计元数据只允许记录数量、
状态和安全 ID，不记录完整授权码或教师备注原文。

`TrialApplication` 保存本地公开表单提交的最小正文：称呼、联系方式、课程类别、视频当前状态、可选 B 站课程链接、教学问题、字幕情况、可选验证问题、提交时间、来源和安全关联信息。课程类别是文本，不冻结分类枚举；B 站链接可为空，填写时只做 `bilibili.com`/`b23.tv` 的简单格式校验，不抓取视频信息。

`TrialFollowup` 是与申请正文分离的运营对象，只保存本地申请引用、封闭跟进状态（`pending`、`contacted`、`closed`）、更新者与时间，以及可选 `teacher_id` 弱关联。申请正文只由获准管理员通过受保护会话读取；跟进状态不自动创建教师账号，也不随账号状态变化被重写。重复申请保留为独立申请，不覆盖已有记录。
...
`TeacherCourseFile` 是自描述的 input/output 契约，不是数据库实体。当前 v2 载体为 `.kmcourse` ZIP，包含文件格式版本、来源类型（已保存草稿或
发布版本）、来源课程/发布的参考信息、课程结构、课节、B 站视频引用、节点教学语义、教师已保存的字幕原文，以及节点图片、音频和辅助视频的
二进制资源（`DATA-PORT-003`、`D-V1-014`）；不包含 B 站主视频本体、密码、会话、授权、学生数据或页面未保存内容。
...
| 发布版本与课程包 | `DATA-DELIVERY-001`、`DATA-DELIVERY-002`、`DATA-DELIVERY-007`、`DATA-DELIVERY-008`、`DATA-DELIVERY-009` | `SRC-022` 至 `SRC-026`、`SRC-034`、`SRC-035` |
...
以下条件已满足，本文件冻结为 `1.1.0`：

1. 第 14 节两项工程取舍已完成人工确认；
2. 每个权威对象、临时对象和派生结果只有一个责任位置；
3. 当前课程发布快照、课程级授权和学生本机学习事实能够由模型无歧义表达；独立课程版本、版本级授权和课程族升级关系只作为后续扩展边界记录；
4. 服务端模型可保存教师已保存的字幕原文（草稿与发布快照）；学生交付课程包和本机模型不包含字幕，教师 `.kmcourse` 可携带已保存字幕；这些文件均不含学生学习数据，本机模型不形成跨设备同步承诺；
