...
| 对象 | 稳定身份与关键字段 | 基数与约束 | 生命周期/分类 |
| --- | --- | --- | --- |
| `Course` | `course_id`、`workspace_id`、标题、说明/目标、修订、生命周期状态、交付暂停状态、时间戳 | 一个工作空间多门独立课程；每个已发布课程版本都是一个独立根对象，直接拥有有序课节 | 身份不随标题或课节编辑变化；新版本不得复用旧版本 `course_id`；归档与内容争议暂停分开 |
...
| 对象 | 稳定身份与关键字段 | 关键规则 |
| --- | --- | --- |
| `CourseRelease` | `release_id`、`course_id`、课程族/来源关系（后续）、发布意图 ID、课程级元数据快照、发布时间、发布教师、源修订摘要、预览记录、权利确认、Schema 版本 | 当前课程的一次原子发布父事实；创建成功后不可变；授权码按 `course_id` 自动解析最新可交付快照；独立 `course_id` 拆分后置 |
...
发布意图仍需幂等，不能因重复请求产生两个相同版本。任一草稿缺失、修订变化、当前修订未完成真实预览、内容权利
门禁失败或快照写入失败时不产生发布。当前发布不改变授权码的 `course_id` 绑定；独立课程身份、草稿消费和版本操作后续另行设计。

教师对已发布课程执行“修改本版本”或“增加版本”属于后续版本操作能力，本阶段不通过现有普通端点组合实现。
...
| 对象 | 稳定身份与关键字段 | 关键规则 |
| --- | --- | --- |
| `AccessCode` | `access_code_id`、工作空间/管理锚点、凭证摘要、显示尾号、领取起止时间、使用上限、状态、创建者与时间 | 使用上限默认允许 20 个不同 `LocalIdentity`；到期与终止分开；数量达到上限后不接受新的标识 |
