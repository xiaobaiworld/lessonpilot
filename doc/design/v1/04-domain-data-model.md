...
flowchart LR
  ...
  Validate --> PublishedCourse[新的独立已发布课程]
  PublishedCourse --> Release[CourseRelease 快照]
  Release --> Snapshots[ReleaseLessonSnapshot × N]
  Snapshots --> Package[按版本级授权范围投影课程包]
```

| 对象 | 稳定身份与关键字段 | 关键规则 |
| --- | --- | --- |
| `CourseRelease` | `release_id`、独立 `course_id`、课程族/来源关系（后续）、发布意图 ID、课程级元数据快照、发布时间、发布教师、源修订摘要、预览记录、权利确认、Schema 版本 | 新课程版本的一次原子发布父事实；创建成功后不可变；`course_id` 不与其它版本复用 |
...
数据库必须保证每个已发布课程版本的 `course_id` 唯一，并在一个事务内写入新课程身份、父发布及全部课节快照。
发布意图仍需幂等，不能因重复请求产生两个相同版本。任一草稿缺失、修订变化、当前修订未完成真实预览、内容权利
门禁失败或快照写入失败时不产生发布。成功发布后消耗当前草稿；已发布课程的内容不能原地修改。

教师对已发布课程执行“修改本版本”时，事务结果是移除该已发布课程并建立草稿；执行“增加版本”时，原课程继续
保留，同时建立一份独立草稿。前者不是历史版本归档，后者也不是在原课程身份下写入新内容。
...
| `GrantItem` | `grant_item_id`、`access_code_id`、独立课程/课节/节点范围、现实有效起止时间 | 范围层级必须自洽；根对象必须是明确的已发布课程版本；课节属于该课程，节点属于该课程的课节；随授权码创建后不可原地改范围 |
...
资格计算必须分别回答：是否可以首次领取、重新下载和更新。当前版本级授权码只回答所绑定独立课程的交付资格；
终止或到期只使对应来源不再贡献未来在线资格，其它来源继续有效，已经安装在学生设备的课程也不被远程改写。
跨独立课程版本的升级资格、旧授权码默认看新版本和更新行为，待 `CourseFamily`/`CourseUpgradeRelation` 完成后再实现。
...
3. 独立课程版本发布、版本级授权和学生本机学习事实能够由模型无歧义表达；课程族升级关系只作为后续扩展边界记录；
