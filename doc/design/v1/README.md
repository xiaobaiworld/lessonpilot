...
状态：01–09 均已通过人工审核并冻结；2026-08-27 按 `D-V1-018` 记录后续独立课程版本方向，并按 `D-V1-019` 同步当前课程级最新版本授权；2026-08-28 提交 `D-V1-020` 授权码管理增强设计，待本次实现前评审；2026-09-01 按 `D-V1-027` 同步本地试用申请设计；2026-09-01 增补 11 视觉设计系统与四端 UI 收敛方案（方向经三方案示意图对比后选定 A 纸绿收敛，阶段 0 token 对比度校准已完成，历史裸色值暂存规则见 `D-V1-031`）；2026-09-03 增补 12 课程包格式与管理员导入导出设计；测试计划与开发计划已建立；当前处于 v1 实现阶段 0（工程基线与干净初始化）
...
| 12 | [`12-course-package-format.md`](12-course-package-format.md) | `.kmcourse` 格式、节点资源边界、管理员导入导出和未来教师能力策略 | 已接受；当前实现管理员代目标教师操作 |
