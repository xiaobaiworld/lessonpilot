...
状态：已于 2026-08-22 通过人工审核；2026-08-27 增补教师课程列表聚合指标、学生作答统计预留字段及节点预览消费边界；按 `D-V1-018` 增补版本级授权和后续升级兼容契约边界；2026-09-01 按 `D-V1-027` 增补本地公开申请接口；本文件把已接受接口需求落为可实现的跨边界契约，不替代业务需求、数据模型或安全运维设计
...
6. 公开销售页、本地试用表单、服务端和人工运营之间的公开申请边界。
...
```text
公开销售页 ──本地公开申请 HTTP──> FastAPI + SQLite ──管理员会话读取──> 管理员/运营
...
| GET | `/api/v1/admin/trial-applications` | 新建 | `FR-INTAKE-003`、`INT-TRIAL-002`、`D-V1-027` |
| POST | `/api/v1/public/trial-applications` | 新建 | `FR-INTAKE-002`、`INT-TRIAL-001`、`D-V1-027` |
...
公开销售页链接到独立的 KnownMap 本地公开试用表单页面；表单页面通过 `POST /api/v1/public/trial-applications` 提交。请求不需要申请人登录；服务端重新执行字段白名单、大小、必填项、B 站链接简单格式、限流和轻量防刷校验。
...
```json
{
  ...
  "courseCategory": "英语口语",
  "videoStatus": "已有课程",
  "bilibiliUrl": "https://www.bilibili.com/video/BVexample",
  "teachingProblem": "希望增加口语练习",
  "subtitleStatus": "已有字幕",
  "validationQuestion": "可选问题"
}
```

成功响应只返回安全申请引用、状态和 `requestId`，例如 `status: "accepted"`；不返回数据库字段，不发送邮件或其它外部通知。申请正文只由管理员会话读取，管理员通过已有跟进端点维护 `pending`、`contacted`、`closed` 三种状态。

申请提交不创建教师账号、会话、工作空间、课程或授权。服务不可用、保存结果未知或请求被限流时，页面不得显示成功，并提供人工联系入口。申请正文不进入普通日志、教师会话、课程正文或学生数据。本阶段不迁移飞书历史数据，也不调用飞书 API。
