...
当前插件已接入第一版当前样式的原创小猫角色包：A1 右下角浮动陪伴形象，提供 `focus`、`idle`、`prompt`、`correct`、`wrong`、`complete` 六个状态，其中 `complete` 是节点完成后的庆祝，声音按小猫声音家族绑定，`idle` 默认静音。插件 `1.2.0` 的弹窗提供紧凑的功能总览首页与插件设置中心；设置中心管理首页显示、课程升级、学习伙伴、声音和维护入口，学习伙伴仍通过独立编辑页展示真实主头像、六个状态缩略图和悬停大图。元气狗狗伙伴、奇趣森林伙伴、未知世界伙伴以及自定义头像仍标记为下一版本。
