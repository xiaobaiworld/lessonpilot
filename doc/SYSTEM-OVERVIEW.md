...
当前插件已接入第一版当前样式的原创小猫角色包：A1 右下角浮动陪伴形象，提供 `focus`、`idle`、`prompt`、`correct`、`wrong`、`complete` 六个状态，其中 `complete` 是节点完成后的庆祝，声音按小猫声音家族绑定，`idle` 默认静音。插件 `1.1.1` 的弹窗品牌栏右上角提供齿轮入口，打开紧凑的“学习伙伴”设置页；设置页以角色类别为主，只开放“神秘猫精灵”，并用真实大图和六个状态缩略图展示当前角色包，悬停缩略图可查看对应大图。元气狗狗伙伴、奇趣森林伙伴、未知世界伙伴以及自定义头像仍标记为下一版本。
