...
**授权码不采用一次性显示规则**：数据库仍只保存授权码摘要，但所属教师可以在授权码管理页
重新查看完整码。学生端只接收兑换结果，不会获得教师侧的授权码管理信息。
...
当前插件已接入第一版当前样式的原创小猫角色包：A1 右下角浮动陪伴形象，提供 `focus`、`idle`、`prompt`、`correct`、`wrong`、`complete` 六个状态，其中 `complete` 是节点完成后的庆祝，声音按小猫声音家族绑定，`idle` 默认静音。插件 `1.2.1` 的弹窗提供紧凑的功能总览首页与插件设置中心；学习伙伴设置在同一 popup 内展示角色类别、真实主头像、六个状态缩略图和五段声音试听。元气伙伴、森林伙伴、未知世界伙伴以及自定义头像仍标记为下一版本。
...
一句话：阶段 0–6 的代码与自动化门禁完成，学生端既有流程已在真实 Chrome 验收；陪伴形象与角色声音包的第一版代码接入、资源构建和自动化验证完成，真实 Chrome 的视觉/听感验收仍待执行。
