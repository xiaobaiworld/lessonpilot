...
```
老师：导入 B 站链接和字幕 → 在某句话上放一个互动节点 → 发布独立课程版本 → 针对版本拿到授权码
 ...
 └─ 发布
      └─ 独立已发布课程版本 Course（拥有新的 course_id）
           └─ 课节 Lesson（绑一个 BVID，显式 sort_order）
                └─ 课程包（不可变快照，学生下载的就是它）
                     └─ 版本级授权码 AccessCode（只存 HMAC 摘要，明文只在创建那一次响应里）
```
...
不同发布版本按不同课程处理。已发布版本支持“修改本版本”（退回草稿并移除原发布）和“增加版本”（保留原发布并创建新草稿）；课程族升级关系和旧授权码默认查看新版本、更新课程属于后续能力，本阶段不实现。
