...
成功返回发布版本、发布时间和可下载配置摘要。响应不返回授权码。
...
| `COURSE_NOT_AVAILABLE` | 404 | 授权码对应课程当前没有已发布配置 |
| `CONFIG_INVALID` | 409 | 已发布配置不符合插件契约 |
| `RATE_LIMITED` | 429 | 后续公网阶段按需要启用；本地阶段可先记录接口边界 |
