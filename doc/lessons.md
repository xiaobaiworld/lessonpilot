...
## 2026-08-27 — 课程版本身份必须先于授权兼容设计

当产品把不同版本视为不同课程时，不能继续沿用“同一课程下的最新发布版本”作为授权和更新的默认语义。先把独立课程身份、草稿消费、“修改本版本”和“增加版本”的保留结果，以及版本级授权码写入需求和数据设计，再把课程族升级与旧授权码兼容单独列为后续能力，避免当前实现偷偷承诺跨版本更新。

本次只同步文档，没有修改代码或数据。下次开发前应先用失败测试锁定四条边界：发布成功消耗草稿、修改本版本不保留原发布、增加版本保留原发布、授权码绑定选中的独立课程版本。
