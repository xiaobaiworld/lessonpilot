...
## 2026-08-25 — 兼容下载名必须与 V1 包逐字一致

线上发布同时保留 `knownmap-v1.zip` 和兼容路径
`knownmapplugin.zip` 时，两个文件名不是两个产品。旧发布脚本先从旧
`src/` 生成 `knownmapplugin.zip`，再从 `v1/extension/` 生成
`knownmap-v1.zip`，导致同一次 `v1-apps` 发布实际交付两套插件：前者是
`0.9.2`，后者是 `1.0.x`。发布只检查 HTTP 200，因而这类错位没有被发现。

以后 V1 发布必须从同一份 V1 ZIP 复制两个下载名，并在生产探针中同时检查：
两个 URL 内容摘要相同、包内 `manifest.json` 使用 `popup/index.html` 和
`content/index.js`，且线上摘要等于本次 release 记录。固定本地测试目录也必须
使用删除同步，不能把新产物覆盖到旧插件目录上继续保留旧文件。
