...
状态：已于 2026-08-22 完成人工审核并冻结；随后按 `D-V1-010` 修订课节内容重复与同视频多课节规则并重新冻结；迁移范围按 `D-V1-012` 同步；2026-08-24 经产品负责人确认增补并扩展 `FR-LIB-014`；2026-08-27 按 `D-V1-018` 记录后续课程版本方向，并按 `D-V1-019` 冻结当前课程级最新版本授权规则；2026-08-28 在功能需求中补充学生学习陪伴形象与其他教师课程推荐两个后续候选；2026-09-01 按 `D-V1-027` 将公开试用申请改为本地表单并同步字段、权限、保留和防刷边界
...
| 01 | [`01-product-scope.md`](01-product-scope.md) | 产品目标、成功标准、利益相关者、范围、边界、非目标、约束和假设 | 已接受；`D-V1-001` 至 `D-V1-010`、`D-V1-018`、`D-V1-027` 已同步 |
| 02 | [`02-domain-glossary.md`](02-domain-glossary.md) | 统一角色、课程、课节、节点、授权、学习记录等领域术语 | 已接受；已同步当前课程级最新版本授权及后续独立课程版本术语 |
| 03 | [`03-user-scenarios.md`](03-user-scenarios.md) | 用户角色、前置条件、主流程、异常流程和可观察结果 | 已接受；同视频多课节、当前课程级授权、后续课程版本和本地试用申请场景已同步 |
| 04 | [`04-functional-requirements.md`](04-functional-requirements.md) | 按业务能力编号的功能性需求、业务规则和单项验收条件 | 已接受；已同步当前课程级最新版本授权，本地试用申请字段、成功状态、三档跟进和轻量防刷，课程版本操作、版本级授权和后续升级兼容后置；新增并澄清 `FR-LIB-015`、`FR-LIB-016` 后续候选；`FR-REPORT` 保留为后续候选 |
| 05 | [`05-data-requirements.md`](05-data-requirements.md) | 数据归属、分类、生命周期、质量、保留、导入导出和删除要求 | 已接受；已同步课程级最新版本授权、本地申请正文权威、无历史迁移和 90 天保留，独立课程身份、版本操作和版本级授权后置 |
| 06 | [`06-interface-integration-requirements.md`](06-interface-integration-requirements.md) | Web、插件、B 站、课程包和文件之间的项目特有接口边界 | 已接受；多课节匹配和本地公开申请接口边界已同步 |
