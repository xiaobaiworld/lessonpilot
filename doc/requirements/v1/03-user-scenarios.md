...
状态：全文已通过人工审核；2026-08-22 按 `D-V1-010` 修订课节内容重复与同视频多课节场景并重新冻结；2026-08-27 按 `D-V1-018` 增补课程版本管理和版本级授权场景；2026-09-01 按 `D-V1-027` 更新本地试用申请场景
...
- **前置条件**：公开销售页可访问；页面展示的能力与当前正式交付一致；销售页到独立 KnownMap 本地申请页面的入口或明确人工联系恢复方式可用。
- **触发**：目标老师决定申请用一节真实课程试用 KnownMap。
- **主流程**：老师从销售页点击入口进入独立的、无需登录的本地表单页面；看到信息用途、必填与可选项；提交称呼、联系方式、课程类别、视频状态、最想解决的教学问题和字幕情况，可选提交 B 站课程链接及产品验证问题；B 站链接只做简单格式校验；服务端成功保存后显示明确成功结果；授权运营人员在管理员后台取得申请并人工联系；确认接入后由管理员另行创建教师账号。
- **异常分支**：表单不可访问、必填项缺失、B 站链接格式不符合、限流、防刷拦截、提交失败或无法确认保存结果时不显示成功；销售页提供明确人工联系入口；重复提交保留为独立申请，不覆盖既有申请。
...
- **前置条件**：课程至少有一个课节；真实预览属于可选的制作辅助流程。
- **触发**：教师准备交付或更新课程。
- **主流程**：教师可以先启动指定草稿的真实预览并检查节点触发、反馈和继续学习，也可以直接从课程详情页或教师工作台点击“发布课程”；系统按当前课程模型生成已发布课程版本。独立课程身份和草稿消费属于后续版本管理能力。
- **异常分支**：预览环境缺失、视频不匹配或触发失败只影响预览，不阻止满足“至少一节课节”条件的发布；空课程、无权访问或发布事务失败时不产生部分发布版本，也不改变其它已发布课程。
- **可观察结果**：教师能区分草稿和已发布课程；预览数据不进入真实学生统计；发布成功后课程进入已发布区域。

### SCN-TCH-05：创建并交付课程授权

- **主要角色**：教师。
- **目标**：把明确版本和范围的已发布课程交付给学生。
- **前置条件**：目标课程已发布；教师有权管理该课程；授权范围和有效时间可确认。
- **触发**：教师准备邀请学生学习。
- **主流程**：教师选择一个已发布课程及其课程、课节或允许的内容范围；确认生效和到期规则；创建绑定该 `course_id` 的课程级授权码；服务端在兑换或更新时自动解析该课程最新可交付版本；安全复制并交付学生。
- **异常分支**：范围跨越其他工作空间、目标尚未发布、范围为空、授权码生成失败或教师重复提交时，不产生越权或重复授权结果。
- **可观察结果**：授权码原文只在允许时出现；教师可以辨认授权码对应的课程；服务端返回的兑换/更新结果带有实际 `release_id`；服务端保留可审计的授权项和状态，不把授权码当作学生身份。
...
- **主流程**：教师选择已发布课程版本的“修改本版本”或“增加版本”；前者把该版本退回草稿区并移除已发布版本，后者保留原已发布版本并创建新的草稿；教师也可以发布草稿形成新的独立课程版本，或查看状态变化的影响后确认归档/终止；历史授权与操作记录按规则保留，未来学习证据不得被当前操作预先删除。
- **异常分支**：并发修改、重复终止、仍存在其它有效授权来源或学生设备离线时，系统不得错误删除其它资格或承诺收回本地内容。
- **可观察结果**：教师能够区分课程归档、账号停用、授权终止和学生本地已有版本。

### SCN-TCH-09：管理课程版本并生成授权码（版本级能力后置）

- **主要角色**：教师。
- **目标**：在工作台清楚管理正在制作的草稿和已经交付的课程，并把课程级授权准确发给学生；独立版本管理后置。
- **前置条件**：教师已进入自己的工作空间；目标课程属于该工作空间；课程已经有可交付发布版本。
- **触发**：教师查看互动课程制作工作台，或准备交付某个已发布课程版本。
- **主流程**：教师看到“互动课程制作”和固定说明文字；点击草稿课程标题进入修改页；点击“发布课程”按当前内容生成已发布课程版本；在课程上选择“生成授权码”；生成授权码时系统绑定 `course_id`，学生兑换或更新时自动取得该课程最新可交付版本。
- **异常分支**：目标课程无权访问、发布校验失败或授权码生成失败时，不产生错误版本、不误删原课程、不生成越权授权码。
- **可观察结果**：教师能够确认授权码对应的课程；服务端返回的兑换/更新结果包含实际使用的最新 `release_id`。独立版本和版本操作属于后续能力。

版本升级与旧授权码兼容属于后续场景：教师可以建立课程升级关系，旧授权码默认可看到同一课程族的新版本并更新；本场景当前只记录，不进入本次实现。
...
- **触发**：教师选择导出课程，或管理员在课程包管理区选择目标教师并导入/导出课程包。
- **主流程**：有权限的操作者确认课程和版本并下载 `.kmcourse`；导入时系统先校验并展示课程摘要与影响；确认后建立新的课程草稿，不覆盖原课程，也不自动发布或创建授权。课程包保存节点图片、音频、辅助视频二进制和文字，但只保存 B 站主视频引用。
- **异常分支**：文件损坏、格式版本不支持、内容越权或写入失败时，不建立部分课程，也不修改现有课程。
- **可观察结果**：重新导入后课程结构、互动语义和节点资源保持一致，但新课程具有新的平台身份；文件不包含 B 站主视频本体、密码、授权码或学生数据。
...
- **触发**：学生收到老师交付的授权码。
- **主流程**：学生打开明确的课程入口；运行时在当前机器建立或恢复本机学习标识；学生输入授权码；服务端校验授权范围；课程包校验通过后自动保存到当前机器。
- **异常分支**：授权码无效、未生效、到期、已终止、范围无已发布内容、网络失败或课程包校验失败时，不覆盖学生课程库，表单在有限时间内恢复可操作。
- **可观察结果**：学生课程库出现经过校验的课程和课节，不显示内部 UUID 代替名称；不下载 B 站播放视频，节点媒体按课程资源清单处理。
...
- **主流程**：系统根据全部有效授权来源计算更新资格；取得并校验新课程包；显示版本与影响；学生主动发起更新后原子更新；迁移仍由稳定课节和节点标识引用的本地状态。
