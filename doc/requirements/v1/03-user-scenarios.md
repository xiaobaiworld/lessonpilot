...
- **触发**：教师准备交付或更新课程。
- **主流程**：教师启动指定草稿的真实预览；在匹配视频中检查节点触发、反馈和继续学习；返回教师端确认当前内容；执行发布；系统根据当前草稿生成一个拥有独立课程身份的已发布课程版本，并把该草稿从草稿区消耗掉。
...
- **前置条件**：目标课程版本已发布；教师有权管理该版本；授权范围和有效时间可确认。
- **触发**：教师准备邀请学生学习。
- **主流程**：教师选择一个已发布课程版本及其课程、课节或允许的内容范围；确认生效和到期规则；创建绑定该版本的授权码；安全复制并交付学生；之后查看不泄露原文的授权记录。
- **异常分支**：范围跨越其他工作空间、目标尚未发布、范围为空、授权码生成失败或教师重复提交时，不产生越权或重复授权结果。
- **可观察结果**：授权码原文只在允许时出现；教师可以辨认授权码对应的具体课程版本；服务端保留可审计的授权项和状态，不把授权码当作学生身份。
...
### SCN-TCH-09：管理课程版本并生成版本级授权码

- **主要角色**：教师。
- **目标**：在工作台清楚管理正在制作的草稿和已经交付的课程版本，并把授权准确发给学生。
- **前置条件**：教师已进入自己的工作空间；目标课程版本属于该工作空间；生成授权码的目标版本已经成功发布。
- **触发**：教师查看互动课程制作工作台，或准备交付某个已发布课程版本。
- **主流程**：教师看到“互动课程制作”和固定说明文字；点击草稿课程标题进入修改页；点击“发布课程”按当前内容生成新的已发布课程版本；在已发布课程上选择“修改本版本”“增加版本”或“生成授权码”；生成授权码时系统把授权绑定到当前选中的已发布课程版本。
- **异常分支**：目标不是已发布版本、版本归属不符、发布校验失败、版本动作冲突或授权码生成失败时，不产生错误版本、不误删原课程、不生成指向其它版本的授权码。
- **可观察结果**：发布后草稿被消耗；“修改本版本”不保留原已发布版本，“增加版本”保留原已发布版本；教师能够确认授权码对应的具体课程版本。
