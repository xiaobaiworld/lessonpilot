...
状态：场景骨架、`D-026` 单机边界、认证场景增补及视频重复范围澄清
均已于 2026-08-21 通过人工审核；当前不替代既有阶段需求或验收计划
...
- **异常分支**：同一课程内课节重复绑定同一视频、链接无效、来源不支持、视频不可访问或权利确认缺失时，不创建错误绑定，也不覆盖原有效绑定；不同课程可以引用同一视频，但保持独立课程身份。
...
| `SCOPE-07` 教师查看结果（后续候选） | `SCN-TCH-07`（不进入当前阶段验收） |
| `SCOPE-08` 平台运营 | `SCN-ADM-01`、`SCN-ADM-02`、`SCN-ADM-03`、`SCN-ADM-04`、`SCN-ADM-05` |
| `SCOPE-09` 数据可携带与恢复 | `SCN-ADM-03`、`SCN-ERR-02`；学习证据导出随 `SCN-TCH-07` 推迟 |
