...
| 管理员登录 | 独立 Cookie、active 管理员、有效会话摘要 | AuthService + dependency | 401 |
| 创建教师 | 登录名唯一、昵称非空、临时密码只返回一次 | service + DB 唯一索引 | 409/422 |
| 重置教师密码 | 教师存在；只更新哈希，不恢复停用状态 | service | 404 或成功 |
| 发布课程数 | 只计 `courses.status = 'published'` | SQL 聚合 | 草稿计数为 0 |
...
| 管理员/教师密码原文落库/日志 | 0 次 | Argon2 哈希测试、敏感扫描 |
| 临时教师密码持久化 | 0 次 | 服务/API 集成测试 |
| 草稿课程误计为已发布 | 0 次 | 聚合查询测试 |
...
- `admins.password_hash` 和 `teachers.password_hash` 只保存 Argon2 哈希；
- 管理员和教师会话只保存 HMAC-SHA256 摘要；
- 管理员初始密码不进入 Settings、`.env.example`、Git 或长期环境文件；
- 创建/重置教师返回的临时密码不得写入日志、操作记录、浏览器存储、URL 或发布记录；
- 页面刷新、退出或管理员会话失效后必须清空临时密码文本和内存状态；
- 教师登录名冲突必须保持原密码哈希、昵称和状态不变。
...
- 日志不出现管理员密码、教师临时密码、密码哈希、Cookie 或原始会话 token。
