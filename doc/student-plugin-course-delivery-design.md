...
状态：插件 `0.9.1` 已实现核心领取、单课程保存、匹配视频运行和工具栏首页；完整真实 Chrome 边界验收待收口
...
1. **工具栏首页**：学生左键点击 Chrome 工具栏中的 KnownMap 图标即可看到授权码输入、当前课程和教师入口；这也是不依赖当前网页的稳定领取入口。
2. **平时陪伴**：可拖动的小助手常驻 B 站页面边缘，响应播放和课程状态，不持续打扰。
3. **学习控制**：点击小助手后展开当前字幕句、上一句、下一句、播放/暂停、单次复听、固定三次复听和持续循环。
4. **页面书包**：在 B 站页面内展示授权码输入、当前课程、有效学习概览、待复习清单和设置。

工具栏首页采用角色分流，而不是伪造账号能力：

- 学生区域明确显示“使用老师发来的授权码，无需注册”，直接完成领取；
- 当前课程读取插件本地唯一的 `installedCourse`，显示完整标准 B 站链接和打开操作；
- 教师区域提供“教师登录”按钮，打开当前已有的教师登录页；
- 学生账号、跨设备身份与学生登录表单仍属于后续阶段，当前界面不得显示无法提交的占位登录。
...
领取成功后，书包的“课程”区域从空状态变为一条当前课程记录；记录必须同时显示这条标准地址的完整可点击文本和“打开课程视频”操作。未安装课程或视频标识未通过校验时只显示空状态，不能生成空记录，也不能把内部 `courseId` 冒充为课程链接。测试版仍只保存一门当前课程，新课程按既有覆盖确认规则替换这条记录，不在本阶段扩成多课程列表。
...
Chrome 消息通道还必须按不可信输入处理：`sendMessage` 拒绝、返回 `undefined`、返回非对象或超过有限等待时间，都统一转为 `EXTENSION_UNAVAILABLE`。页面书包和工具栏首页必须恢复提交按钮并提示学生重新加载插件和页面，不能留下永久 loading 状态。
...
- 多老师授权和授权并集；
- 学生账号、Google 登录和设备绑定；
- 授权码停用、人数限制和一次性使用；服务端已支持短期 7 天和长期不过期，插件只处理统一的有效/无效响应，不管理有效期；
- 领取记录和学生关系；
- 学习数据上传和教师报告；
- AI 动态教学；
- 声音控制正式交付；
