...
状态：已于 2026-08-23 审核采用同仓库 `v1/` 目录隔离替换策略；本文件是唯一逐工作包实施真源，
但仍不授权修改产品代码、部署或生产数据

`0.3.0` 吸收 [`v1-replacement-plan.md`](v1-replacement-plan.md) 的已接受决策：新系统全部进入 `v1/`，
旧系统冻结功能和结构变更；检查、测试和发布工具在过渡期必须显式覆盖两套目录；v1 切换后另设阶段 8
清零消费者、恢复与追溯责任，再删除旧系统。替换计划保留决策依据，不再部分覆盖本文件章节。
...
本轮在同一仓库新建 `v1/`，按目标边界从干净初始化建立新系统；旧 `backend/`、`teacher-web/`、`src/`
保持可运行基线，不在原目录逐文件改造成 v1。已验证规则通过特征测试和精确来源提交迁入，不直接继承旧目录结构。
...
旧系统冻结新功能和结构重构；只有 P0、数据/凭证安全和生产可用性问题可以附回归测试修复，
并检查同一缺陷是否存在于 v1。v1 与旧系统不双写业务数据，也不在没有版本支持矩阵时同时对外提供同一业务能力。
...
v1/
  ...
  extension/
    ...
    host/bilibili/
    storage/
  tests/                     # v1 单元、集成、契约、构建产物测试

tests/
  fixtures/v1/               # 新旧工具共用的匿名契约夹具
  manual/v1/                 # 真实浏览器、发布、恢复和交付证据

backend/  teacher-web/  src/ # 冻结旧系统；阶段 8 前不删除
doc/  tools/                 # 仓库级真源与同时覆盖 legacy/v1 的工程门禁
```

销售页先留在现有轻量入口；旧系统不整体搬进 `legacy/`，避免无业务价值的大规模重命名。
旧入口在 v1 完成候选验证前继续承担原责任，阶段 7 原子切换，阶段 8 才物理删除。
...
1. `0A` 建立 `v1/` Node workspace、TypeScript/Vite/React 最小依赖锁、`v1/web`、`v1/extension`、
   `v1/contracts`、`v1/backend/app/modules` 空骨架；旧页面、旧后端和旧插件继续可运行。
...
6. `0F` 改造仓库级工程门禁：根 `npm test` 同时编排旧测试、文档检查和 v1 测试；
   `endpoint-check`、`contract-check`、`module-check`、`dependency-check` 显式识别 v1 路径和依赖根；
   `doc-check`、`secret-scan`、`build-traceability` 验证扫描范围覆盖 `v1/`。每个检查器用临时违规夹具证明
   “v1 违规会失败”，防止只检查旧目录的假绿。
7. `0G` 把 v1 Ruff、TypeScript typecheck、Node/Python 测试、契约生成无 diff、秘密和依赖扫描接入 CI；
   旧后端/旧 Node 测试继续作为冻结基线，v1 后端使用独立声明、锁文件、数据库和测试命令。
...
6. `1F` 按设计 03 第 7.0 节表归属建立 v1 零豁免模块门禁。旧系统已登记的 9 处跨模块访问只作为
   反例和参考修法，不在冻结旧后端里重构，也不能复制为 v1 白名单：
...
7. `1G` `tools/module-check.mjs` 对 `v1/backend/app/modules/<domain>/` 按目录判定，新增跨模块表访问立即失败。
   §2 的 `CourseDetail.lessons[]` 与旧 `app.js` 读取 `course.lesson` 断点保留为特征测试；v1 Web 只消费
   OpenAPI 生成/校验的 `lessons[]`，不修改旧页面来掩盖基线缺陷。

门禁：两个教师的交叉资源访问全部拒绝且无副作用；停用立即使现有会话失效但不删业务数据；重复内容和同视频课节合法；草稿冲突可恢复；
`node tools/module-check.mjs` 对 v1 无任何越界或豁免；旧系统 `KNOWN_VIOLATIONS` 数量未增加；
...
5. `6E` 隔离旧数据库、旧 storage、旧 API/页面/诊断入口；阶段 6/7 只读、拒绝或重定向，不物理删除。

门禁：全量 `SEC-*`、`OPS-*`、`MIG-*` 通过；候选失败可回到完整上一组合，不允许只回退 Web 或只回退 API；恢复验证证明备份可读且业务对象对账一致。

### 阶段 7：真实验收与发布

按 [`v1-test-plan.md`](v1-test-plan.md) 和 `D-V1-005` 完成真实老师、真实学生、第二次独立交付、重启恢复、更新、失败恢复、备份恢复和回退。257 个需求编号只有获得可重复证据后才能改为“已验证”。
通过后切换匹配的 Web/API/插件/Schema 组合，但保留上一套完整发布组合和旧源码回退入口。

### 阶段 8：观察、责任清零与旧系统退役

1. `8A` 在预先记录的观察期内完成至少一个完整真实交付周期，核对健康、错误、版本、备份和恢复结果。
2. `8B` 清点旧 API、页面、插件消息、存储键、书签、发布脚本、文档和人工支持消费者；逐项证明为零或已有明确拒绝/重定向。
3. `8C` 证明旧系统不再承担回退、恢复、追溯或安全处置责任；历史源码、发布摘要和 `SRC-*` 证据入口仍可检索。
4. `8D` 通过独立 PR/MR 删除旧 `backend/`、`teacher-web/`、`src/`、旧专用工具和 35 个旧测试文件；
   同步根 README、索引、开发规则、发布清单和 changelog。

门禁：删除提交不包含首次 v1 切换；删除前后仓库总门禁、v1 构建、发布探针和恢复验证均通过；
删除后仍能从固定提交和发布记录审计旧系统，但生产运行时不存在旧入口或未知消费者。
...
| 旧系统退役 | 尚有书签、插件或恢复责任 | 消费者清单 + 回退演练 + 删除后探针 | 阻止删除并保留旧入口 | 不因清理代码破坏真实用户或恢复路径 |
...
阶段完成后创建 PR/MR，执行整阶段文档审计和独立工程审查；CI、文档、风险和用户授权确认前不合并。不要把 9 个阶段合成一个长期重构分支。
