...
> 归档说明：本文件是教师平台节点 1–7 完成时的执行记录，只用于追溯。当前执行入口为仓库根目录 `next.md`。
...
- [x] 核对 `knownmap.com` 当前线上文件与 GitHub 提交；
- [x] 确认销售页公网白名单和回滚边界；
- [x] 实现 GitHub SHA 固定、服务器历史、生产标签和原子回滚；
- [x] 使用新流程重新发布当前版本并补齐记录；
- [x] 完成线上查询、拒绝路径和回滚目标验证。
