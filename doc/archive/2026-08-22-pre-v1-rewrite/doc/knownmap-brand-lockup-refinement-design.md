...
- 四份 SVG 均使用 `translate(23.4 23.4) scale(0.82)` 的共享内部几何；
- 16、24、48、128 像素插件 PNG 和网页 48 像素 PNG 已重新导出；
- 教师应用标题、登录文案、运行时标题和课程创建说明统一为“KnownMap 互动课程工具”；
- 页眉 K/M 计算色值分别为 `rgb(217, 165, 30)` 和 `rgb(169, 101, 78)`；
- 1440 x 900 与 375 x 812 浏览器检查无页面级横向溢出或控制台错误。
