...
- 创建 `KnownMap 真实课程互动改造申请` 飞书表单，基础联系字段与 D-013 的两个未来方向问题按当前决策收口。
- 销售页增加独立的 `trial-intake.js` 次 CTA 模块；只接受飞书/Lark 官方 HTTPS 地址，链接缺失或无效时整块隐藏。
- GitHub Pages 和可追踪生产发布白名单同步加入该模块，避免根路径与兼容路径出现脚本缺失。
- 新增字段、权限、数据边界、维护和匿名提交验收文档，以及对应单元、页面文案、信息架构和发布构建测试。
