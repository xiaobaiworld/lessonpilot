...
状态：**V1 已通过，V2–V7 待执行**。执行者填写全部「实际结果」列后，1A 才可标记完成。
...
| 预期 | 实际结果 |
| --- | --- |
| `node --test tests/*.test.js` 全部通过，无失败 | **通过**（2026-08-15，提交 `946d85a`）：135 pass / 0 fail，耗时 126ms。同时 CI `node-test` 在 PR #1 上 pass。 |

V1 不依赖真实浏览器，已由自动化直接完成。V2–V7 必须在你自己的机器上执行。
