...
### 已完成切片：教师工作台与学生网页最小闭环

- 结果：`teacher-web/` 首页改为当前课程、待确认建议、课程健康度和继续设计入口；`student-web/` 从 `course.json` 读取固定 B 站样例，学生主路径只展示课程、进度、视频、练习、反馈和本地学习总结。
- 已验证：桌面与 375px 移动宽度无横向溢出；本地 MP4 依次触发两题并完成 `2 / 2` 总结。
- 边界：B 站 iframe 与本地视频选择均收纳在测试者工具，只用于课程来源和播放器兼容性验证。跨站播放器不作为网页端读时、暂停或弹题的正式能力承诺；正式可控体验暂用测试者选择的本地视频。未接后端、AI、真实视频托管、老师网页保存或 SRT/VTT 导入。

### A. 网页学生端最小闭环（已完成）

1. [x] 新增 `student-web/` 静态学生课堂页，使用 HTML5 video 和固定互动节点。
2. [x] 实现 `pause -> question -> deterministic feedback -> continue`，先做选择题与填空题。
3. [x] 本地记录当前会话答案、尝试次数、完成状态，并展示学习总结。
4. [x] 在手机宽度下验证题卡、视频和继续按钮不互相遮挡。

### B. 当前下一步：为 B 站样例定出首个真实教学节点

5. [ ] 人工查看 `BV1WW4y1e7GL`，记录一个内容匹配的教学片段、时间戳、题目与标准答案；网页中的 1 秒 / 3 秒节点仅用于运行时演示。
6. [ ] 用该节点替换 `student-web/course.json` 中的占位节点，并在网页与现有插件 spike 各人工验收一次。
...
11. [x] 将网页学生端的状态机抽成可复用运行时，再决定是否回填到插件。
12. [x] 本地会话记录尝试次数与答案；预览会话与学生会话字段先预留 `sessionType`。
