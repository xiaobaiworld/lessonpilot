...
3. 建立 v1 目录骨架与职责边界（工作包 `0A`：`v1/web`、`v1/extension`、`v1/contracts`、
   `v1/backend/app/modules` 空骨架，旧页面、旧后端和旧插件继续可运行）；
4. 改造仓库总测试入口和检查工具，使 legacy/v1 都被真实发现，并用临时违规夹具证明 v1 违规会失败。
