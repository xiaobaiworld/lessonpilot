...
- 结果：`student-web/` 从 `course.json` 读取固定 B 站样例，展示原课 iframe 与“在 B 站打开原课”回退；学生选择本地 HTML5 视频后，可完成到点暂停、两种确定性题、反馈、进度和本地学习总结。
- 已验证：桌面与 375px 移动宽度无横向溢出；本地 MP4 依次触发两题并完成 `2 / 2` 总结。
- 边界：B 站 iframe 仅用于确认课程来源和原课呈现。跨站播放器不作为网页端读时、暂停或弹题的正式能力承诺；正式可控体验暂用学生选择的本地视频。未接后端、AI、真实视频托管、老师网页保存或 SRT/VTT 导入。
