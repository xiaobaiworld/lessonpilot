...
- 下一目标：按 `SRC-001` 至 `SRC-082` 提取旧资料；`SRC-001` 至 `SRC-007` 已完成，公开销售与飞书试用申请已按 `D-V1-001` 纳入 v1 P0；当前进入 `SRC-008` `doc/AI_Learning_Companion_Product_Function_Spec_v0.2.md`。机械归类由 Agent 自行处理，只有实质产品决策、冲突和归档动作提交人工判断。
