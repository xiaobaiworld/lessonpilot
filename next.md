...
### 当前切片：W0 教师工作台与学生网页课程壳

- 目标：`teacher-web/` 展示当前课程、待确认建议、课程健康度和继续设计入口；`student-web/` 呈现固定 B 站课程的学习壳和原课回退。
- 边界：网页不出现本地视频选择、上传、托管或测试者播放器替换。跨站播放器不作为网页端读时、暂停、seek 或弹题的正式能力承诺；真实定时互动继续在 B 站原页面插件 spike 中验证。未接后端、AI、课程保存或 SRT/VTT 导入。
- 验收：教师可打开学生预览；学生可进入固定 B 站原课；桌面与 375px 页面无横向溢出或关键动作遮挡。

### A. W0 网页课程壳（进行中）

1. [x] 先完成教师工作台与学生课程页的角色分层设计。
2. [x] 移除网页端本地视频、互动运行时和测试者替换入口，只保留固定 B 站课程呈现与原课回退。
3. [x] 复验教师预览、固定课程呈现和桌面/手机布局。

### B. 当前下一步：为 B 站样例定出首个真实教学节点

4. [ ] 人工查看 `BV1WW4y1e7GL`，记录一个内容匹配的教学片段、时间戳、题目与标准答案。
5. [ ] 用该节点替换插件 spike 中的占位节点，并在 B 站原页面人工验收一次。
