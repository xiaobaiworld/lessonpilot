...
B 站不向自动化浏览器下发 `<video>`，所以「在真实 B 站页面上」这一点只能
人工在日常 Chrome 里看一眼。播放器逻辑本身已由夹具在真实 Chromium 中验过，
剩下的是确认 B 站自己那套 DOM 与选择器仍然匹配——B 站改版时才需要重看。

全屏这一项也只能人工：`requestFullscreen` 需要真实用户手势。
