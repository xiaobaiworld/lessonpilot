...
- 已接受：产品范围、领域术语、用户场景、当前阶段全部功能需求、数据需求、接口与集成需求、第一阶段非功能需求结论、安全隐私、开发质量及部署运维需求；`FR-REPORT-*` 保留为后续候选。
- 第一阶段不单独制定 `NFR-*`；响应、容量、并发、可用率和兼容时限留待获得真实依据后重新审核。
- 下一目标：建立并审核 `11-migration-compatibility-requirements.md` 迁移与兼容需求。
