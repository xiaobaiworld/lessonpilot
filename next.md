...
- 目标：把教师端从固定三节点表单升级为“上传课程 → AI 分析 → 设计课堂 → 模拟学生 → 发布课程”的可点击高保真原型。
- 涉及文件：`teacher-web/index.html`、`teacher-web/styles.css`、`teacher-web/app.js`、`doc/subtitle-pipeline.md`。
- 验证方式：本地静态服务器打开页面；检查首页和课堂设计页面；点击字幕段落、采纳 AI 建议、输入教学意图、生成课堂设计和模拟学生课堂；确认控制台无错误。
...
- 颜色系统：保留暖纸白 + 墨绿品牌方向；四类课堂事件使用低饱和黄、青绿、陶土和灰青；状态色与注意力色分离，AI 不使用紫色。详见 `doc/ui-design.md`。
- Creator Studio 调整：依据 `LessonPilot_Creator_Studio_设计建议_v0.1.md`，产品叙事从“字幕时间线编辑器”升级为“AI 先备课、老师审核的课堂设计工作台”。页面增加课程分析状态、知识点/易错点、固定 AI Copilot、采纳/忽略建议、自然语言教学意图和课堂模拟入口；真实 AI 分析与学生模拟仍不在本切片实现范围内。
