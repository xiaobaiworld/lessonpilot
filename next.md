...
当前状态：节点 1–6 已完成；节点 7 API 接入完成，正在执行可视化节点编辑器修正；修正完成后执行节点 8

## 当前唯一目标

完成节点 7 可视化编辑器修正：用组件注册和横向时间轴替换当前字幕列表主编辑模型，
并同时支持点击放置和拖放放置。

计划入口：`doc/teacher-platform-dev-plan.md`
详细计划：`doc/plans/teacher-visual-node-editor.md`
...
- [ ] V7.4 实现点击放置
- [ ] V7.5 实现拖放创建和节点移动
- [ ] V7.6 实现类型化弹窗、删除和字幕同步
- [ ] V7.7 接回草稿、发布和授权码
- [ ] V7.8 完成日志、可访问性和响应式收口
- [ ] V7.9 完成浏览器和全量回归
- [ ] V7.10 同步文档并转入节点 8
...
- 后端全量测试：37 passed；
- Node 全量回归：207 passed；
- 页面 API 客户端测试：3 passed；
- Python `compileall` 和 Node 脚本语法检查：通过；
- CORS：`http://127.0.0.1:4173` 携带 cookie 的预检通过；
- Playwright 本地验收：登录、读取课程、编辑四种节点、保存草稿、发布课程、创建授权码均成功；
- 发布版本和授权码面板在桌面视口可见；
- 375px 移动视口：页面宽度 375px，无横向溢出；
- 浏览器控制台：无页面脚本错误和非预期 HTTP 错误；
- 截图记录：`/tmp/lessonpilot-node7-published.png`、`/tmp/lessonpilot-node7-mobile-final.png`。
