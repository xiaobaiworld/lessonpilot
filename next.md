...
- 目标：先把 D0 教师端的首页与受限课程编辑器画成可点击的高保真静态网页，验证信息架构和演示路径。
- 涉及文件：`teacher-web/index.html`、`teacher-web/styles.css`、`teacher-web/app.js`。
- 验证方式：本地静态服务器打开页面；检查桌面与移动视口；点击首页双入口、三个节点切换、字段编辑、保存、恢复默认与保存并预览；确认控制台无错误。
- 边界：本切片只交付网页原型，扩展桥、`chrome.storage.local` 和真实视频预览仍按 C.9–C.12 后续接入。
- 结果：已完成 `teacher-web/` 原型；下一步进入 C.8–C.12 的 localhost 老师站真实桥接实现。
