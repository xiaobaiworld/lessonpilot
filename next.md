...
- 下一目标：按 `SRC-001` 至 `SRC-082` 提取旧资料；SRC-001 至 SRC-082 已完成提取，公开销售与飞书试用申请按 `D-V1-001` 纳入 v1 P0，`D-V1-002` 排除 AI 辅助制作，`D-V1-003` 排除字幕导航与学习/复习指标；下一步进行全表一致性核对和开放项清单收口。机械归类由 Agent 自行处理，只有实质产品决策、冲突和归档动作提交人工判断。
