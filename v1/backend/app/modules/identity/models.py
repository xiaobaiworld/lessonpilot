...
from sqlalchemy import Column, String, Integer, DateTime, Boolean, ForeignKey, Enum, Index
...
class AdminAccount(Base):
    ...
    - email is the unique login identifier per administrator
...
class TeacherAccount(Base):
    ...
    # Relationships
    sessions = relationship('TeacherSession', back_populates='account', cascade='all, delete-orphan')
    workspace = relationship('Workspace', back_populates='owner', uselist=False)

    __table_args__ = (
        Index('ix_teacher_accounts_login_name', 'login_name'),
    )
...
class Workspace(Base):
    ...
    - owner_teacher_id is immutable; one teacher = one workspace
    - status tracks active/suspended/archived
    - Business boundary for course ownership and access control
    """
    __tablename__ = 'v1_workspaces'

    id = Column(String(36), primary_key=True)  # UUID
    owner_teacher_id = Column(String(36), ForeignKey('v1_teacher_accounts.id'), unique=True, nullable=False)
    name = Column(String(255), nullable=False)
    status = Column(String(50), default='active', nullable=False)  # active/suspended/archived
    created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), nullable=False)
    updated_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc),
                       onupdate=lambda: datetime.now(timezone.utc), nullable=False)
    ...
    __table_args__ = (
        Index('ix_workspaces_owner_teacher_id', 'owner_teacher_id'),
    )
