...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("draft", "active", "archived", "delivery_paused", name="course_status"),
            default="draft",
            nullable=False,
        ),
    ...
    op.create_unique_constraint(
        "uq_lessons_course_sequence", "v1_lessons", ["course_id", "sequence"]
    )
    ...
    op.create_table(
        ...
        sa.Column("id", sa.String(36), primary_key=True),
        sa.Column(
            "lesson_id", sa.String(36), sa.ForeignKey("v1_lessons.id"), unique=True, nullable=False
        ),
    ...
    op.create_index(
        "ix_video_references_platform_id", "v1_video_references", ["platform", "platform_video_id"]
    )
