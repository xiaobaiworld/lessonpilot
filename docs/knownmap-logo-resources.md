...
| 形态 | 资源 | 适用场景 |
| --- | --- | --- |
| 圆形深绿底 | `v1/brand/knownmap/knownmap-circle.svg` | 网页品牌头像、favicon、需要明确品牌容器的入口 |
| 方形深绿底 | `v1/brand/knownmap/knownmap-square.svg` | Chrome 扩展入口、工具栏、方形应用容器 |
| 透明背景 | `v1/brand/knownmap/knownmap-transparent.svg` | 浅色页面、文档、印刷物、自定义背景和不希望带外框的场景 |
...
- `v1/brand/knownmap-logo.svg`：当前主 Logo 源文件；
- `v1/extension/assets/icon-16.png`、`icon-24.png`、`icon-48.png`、`icon-128.png`：V1 扩展图标；
- `v1/public-site/assets/knownmap-icon.png`：网页图标。

三种形态的 SVG 变体集中在 `v1/brand/knownmap/`。V1 扩展构建会把四种 PNG 图标写入产物，并由 manifest 统一引用。
